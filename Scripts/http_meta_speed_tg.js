...
if (telegram_chat_id && telegram_bot_token && validProxies.length > 0) {
    ...
    try {
        ...
        $.info("Telegram 通知发送成功"); // 新增日志
        $.info(`Telegram 响应: ${JSON.stringify(response.body)}`); // 新增日志
    } catch (error) {
