...
async function operator(proxies = [], targetPlatform, env) {
  ...
  // 修改这里：如果 $arguments.custom 未定义，默认使用 subName 或 '默认订阅'
  ...
  if (telegram_chat_id && telegram_bot_token && validProxies.length > 0) {
    try {
      ...
      // 构建消息内容，并转义括号
      const escapedCustom = custom.replace(/([()])/g, '\\$1');
      const escapedFastestName = fastestProxy.name.replace(/([*()])/g, '\\$1');
      const escapedSlowestName = slowestProxy.name.replace(/([*()])/g, '\\$1');

      const text = `*${escapedCustom}* 订阅测试\n最快节点为：*${escapedFastestName}*\n最慢节点为：*${escapedSlowestName}*\n共\\(${validProxies.length}\\)个节点`;
