...
async function operator(proxies = [], targetPlatform, env) {
  ...
  console.info(`核心支持节点数: ${internalProxies.length}/${proxies.length}`);
  ...
  console.info(`等待 ${http_meta_start_delay / 1000} 秒后开始检测`);
  await new Promise(resolve => setTimeout(resolve, http_meta_start_delay));
  ...
  if (telegram_chat_id && telegram_bot_token && validProxies.length > 0) {
    try {
      ...
      // 错误处理
      console.error(`发送 Telegram 通知失败: ${error.message || error}`);
    }
  }
  ...
  async function check(proxy) {
    ...
    try {
      ...
      console.info(`[${proxy.name}] status: ${status}, latency: ${latency}, speed: ${speed} M`);
      ...
      if (speed >= minSpeed) {
        ...
        if (cacheEnabled) {
          console.info(`[${proxy.name}] 设置成功缓存`);
          cache.set(id, { speed });
        }
      } else {
        if (cacheEnabled) {
          console.info(`[${proxy.name}] 设置失败缓存`);
          cache.set(id, {});
        }
      }
    } catch (e) {
      console.error(`[${proxy.name}] ${e.message ?? e}`);
      if (cacheEnabled) {
        console.info(`[${proxy.name}] 设置失败缓存`);
        cache.set(id, {});
      }
  ...
  async function http(opt = {}) {
    ...
    const fn = async () => {
      try {
        return await (METHOD === 'get' ? fetch(opt.url, { timeout: TIMEOUT }) : fetch(opt.url, {
          method: METHOD,
          headers: opt.headers,
          body: opt.body,
          timeout: TIMEOUT
