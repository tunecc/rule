...
async function operator(proxies = [], targetPlatform, env) {
  ...
  if (telegram_chat_id && telegram_bot_token && validProxies.length > 0) {
    try {
      ...
      const escapedCustom = escapeMarkdown(custom);
      const escapedFastestName = escapeMarkdown(fastestProxy.name);
      const escapedSlowestName = escapeMarkdown(slowestProxy.name);
